<template>
	<view class="page" v-if="list.length" :style="{'bottom':bottomHeight + 'px'}">
		...
		<view class="list" v-if="listShow">
			<scroll-view :scroll-y="true" style="max-height: 60vh;">
				<view class="music" v-for="(item,index) in list" :key="index" :class="musicIndex==index?'musicActive':''"
					...
					<view class="left" v-if="musicIndex==index">
						<image :src="item.pic?baseUrl + item.pic.split(',')[0]:'/static/player/plays.jpg'" mode="aspectFill"></image>
					</view>
					<view class="center">
		...
		<view class="playView">
			<view class="playLeft">
				<image
					:src="musicIndex!= -1&&list.length&&list[musicIndex].pic? baseUrl + list[musicIndex].pic.split(',')[0]:'/static/player/plays.jpg'"
					mode="aspectFill"></image>
			</view>
			<view class="playRight">
				<view class="top">
					<view class="info">{{musicIndex!= -1&&list.length ?list[musicIndex].title:''}} -
						{{musicIndex != -1&&list.length?list[musicIndex].artist:''}}
					</view>
					<view class="play">
		...
		methods: {
			...
			playClick(item) {
				...
				innerAudioContext.onTimeUpdate(() => {
					...
					if(currentTime>30&&this.list[this.musicIndex].isfree==0&&(!user||!user.vip||(user.vip&&user.vip!='是'))){
						innerAudioContext.stop()
						this.playClicks()
					}
					this.$forceUpdate()
