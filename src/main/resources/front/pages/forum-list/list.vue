	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" @scroll="scrollChange">
		...
		<view class="list">
			<view v-for="(item,index) in list " v-bind:key="index" class="listm flex flex-between">
				...
				<view class="listmr">
					<view class="col3 f30 elip mb15">{{item.title}}</view>
					<view class="col3 f30 elip mb15">
						<button v-if="item.isdone=='开放'" class="cu-btn bg-green sm">公开</button>
						<button v-if="item.isdone=='关闭'" class="cu-btn bg-red sm">私人</button>
					</view>
