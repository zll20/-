			...
			<view :style='{"minHeight":"100%","width":"100%","padding":"40rpx 20rpx 198rpx","position":"relative","background":"#F6F7F9","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"0","background":"none","display":"flex","height":"auto","order":"1"}'>
					<view  :style='{"margin":"0","flex":"1","position":"relative"}' class="search-form round">
						...
						<input placeholder-style="color: #999" :style='{"border":"2rpx solid #FFD128","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"12rpx 0 0 12rpx","background":"#fff","width":"100%","lineHeight":"56rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.shifoujizhu" type="text" placeholder="是否记住" ></input>
					</view>
					<button :style='{"border":"0","padding":"0px","margin":"0","color":"#fff","borderRadius":"0 12rpx 12rpx 0","background":"#FFD128","width":"136rpx","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' @tap="search" class="cu-btn shadow-blur round">搜索</button>
				</view>
					...
					<view class="list" :style='{"width":"100%","padding":"0","margin":"40rpx 0 0 0","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"alignContent":"space-between","boxShadow":"0 2rpx 12rpx #dddddd","padding":"20rpx","margin":"0 0 30rpx 0","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"alignContent":"center","padding":"0 20rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"68%","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.xingming}}</view>
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.xuehao}}</view>
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>学习时长:{{product.xuexishizhang}}</view>
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>单词量:{{product.danciliang}}</view>
								<view class="col3 f30 elip mb15" :style='{"padding":"0","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.shifoujizhu}}</view>
								...
								<view :style='{"padding":"0 10rpx 0 0","order":"4"}'>
									...
									<text :style='{"color":"#666","lineHeight":"50rpx","fontSize":"24rpx"}'>{{product.xuehao}}</text>
								</view>
							</view>
							<!-- #ifdef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('dancixuexi','修改')) || (!userid && isAuthFront('dancixuexi','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('dancixuexi','删除')) || (!userid && isAuthFront('dancixuexi','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"20rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('dancixuexi','修改')) || (!userid && isAuthFront('dancixuexi','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#3ab5f5","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#3ab5f5","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('dancixuexi','删除')) || (!userid && isAuthFront('dancixuexi','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#ff0000","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#ff0000","display":"inline-block"}'>删除</text>
								</view>
							</view>
				...
				<button :style='{"border":"0","margin":"20rpx 0 0 10rpx","whiteSpace":"nowrap","color":"#fff","alignItems":"center","bottom":"170rpx","display":"flex","right":"20rpx","justifyContent":"center","outline":"none","borderRadius":"80rpx","background":"#F49843","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"99"}' v-if="userid && isAuth('dancixuexi','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","margin":"20rpx 0 0 10rpx","whiteSpace":"nowrap","color":"#fff","alignItems":"center","bottom":"170rpx","display":"flex","right":"20rpx","justifyContent":"center","outline":"none","borderRadius":"80rpx","background":"#F49843","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"99"}' v-if="!userid && isAuthFront('dancixuexi','新增')" class="add-btn" @click="onAddTap()">新增</button>
				<button :style='{"border":"0","margin":"20rpx 0 0 10rpx","whiteSpace":"nowrap","color":"#ffffff","alignItems":"center","display":"flex","justifyContent":"center","outline":"none","borderRadius":"80rpx","top":"190rpx","left":"10rpx","background":"#ffaa33","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"99"}' v-if="staticType" class="statics-btn" @click="staticClick()">
					统
				</button>
			</view>
		</view>
	...
	export default {
		...
		async onShow() {
			if(uni.getStorageSync("useridTag")==1){
				...
				if(
					this.isAuth('dancixuexi','学习分析')
					||this.isAuth('dancixuexi','学习时长')
				) {
				...
				this.userid = "";
				if(
					this.isAuthFront('dancixuexi','学习分析')
					||this.isAuthFront('dancixuexi','学习时长')
				) {
					this.staticType = true
				}else {
					this.staticType = false
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.shifoujizhu){
					params['shifoujizhu'] = '%' + this.searchForm.shifoujizhu + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`dancixuexi`, params);
                } else {
                    res = await this.$api.list(`dancixuexi`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await that.$api.del('dancixuexi', JSON.stringify([id]));
							that.$utils.msg('删除成功');
							that.hasNext = true
							// 重置分页参数页数为1
							that.search()
			...
			async search(){
				...
				if(this.searchForm.shifoujizhu){
					searchForm['shifoujizhu'] = '%' + this.searchForm.shifoujizhu + '%'
				}
				let res = {};
				if(this.userid) {
					res = await this.$api.page(`dancixuexi`, searchForm);
				} else {
					res = await this.$api.list(`dancixuexi`, searchForm);
				}
				// 如果是第一页数据置空
