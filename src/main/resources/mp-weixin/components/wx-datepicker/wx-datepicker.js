...
Component({
  ...
  methods: {
    ...
    setDateByMode() {
      ...
      let year = wDate.getYear(this.data.date);
      let month = wDate.getMonth(this.data.date);
      this.setDays(year, month);
      let days = wDate.getDay(this.data.date);
      let hours = wDate.getHour(this.data.date);
      let minutes = wDate.getMinute(this.data.date);
      let seconds = wDate.getSecond(this.data.date);
      beforeYear = year;
      beforeMonth = month;
    ...
    setDays(year, month) {
      ...
      if (year != beforeYear || beforeMonth != month) {
        beforeYear = year;
        beforeMonth = month;
        let dayCount = wDate.getDaysOfMonth(year, month);
    ...
    bindChange: function (e) {
      ...
      if(this.data.starttime){
        ...
        let nowtime = this.data.starttime
        if(new Date(currenttime).getTime() <= new Date(nowtime).getTime()){
          this.setData({
            value: [nowtime.split('-')[0] - 1970, nowtime.split('-')[1] - 1, nowtime.split('-')[2] - 1, this.data.value[3], this.data.value[4], this.data.value[5]]
          })
          return false
        }
      }
    ...
    onOkClick() {
      ...
      this.triggerEvent('dateChange', myEventDetail)
    ...
    getResultDate() {
      ...
      let month = this.data.months[resultValue[1]];
