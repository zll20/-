...
Page({
	...
	async onShow() {
		...
		if(this.data.userid){
			this.setData({
				...
				isAuth: utils.isAuth('discussdanciku')
			})
		}else {
			this.setData({
				isAuth: utils.isAuthFront('discussdanciku')
			})
		}
		let cleanType = wx.getStorageSync('discussdiscussdancikuCleanType')
		if(cleanType){
			wx.removeStorageSync('discussdiscussdancikuCleanType')
			this.getDetail(2);
		}
	...
	getDetail(type=1) {
		api.get('discussdanciku/detail/' + this.data.id, {}).then((res)=>{
			let reg=new RegExp('http://localhost:8081/code/upload','g')//g代表全部
			let reg2=new RegExp('<img','g')//g代表全部

			if(type==2){
				res.data.discussnum++
				api.post('discussdanciku/update',res.data).then((rs2)=>{})
			}
			// 数据存储
			this.setData({
				detail: res.data,
	...
	async onAcrossTap(e){
		...
		wx.setStorageSync('crossTable','discussdanciku');
	...
	getCommentList() {
		let params = {
			...
			refid: Number(this.data.id),
			sort: 'istop',
			order: 'desc',
		}
		api.get('discussdiscussdanciku/list', params).then((res)=>{
			res.data.list = res.data.list.map(item=>{
				item.content = item.content.replace(/img src/gi,"img style=\"width:100%;\" src")
				item.comZanType = this.comzanChange(item)
				item.comCaiType = this.comcaiChange(item)
				return item;
			})
			this.data.commentList = this.data.commentList.concat(res.data.list);
			this.setData({
	...
	async comzanClick(e){
		...
		if(!this.comzanChange(item)){
			item.thumbsupnum++
			if(item.tuserids){
				item.tuserids = item.tuserids + ',' + this.data.user.id
			}else {
				item.tuserids = this.data.user.id
			}
			api.post('discussdiscussdanciku/update', item).then((res)=>{
				utils.msg('点赞成功')
			})
		}else {
			item.thumbsupnum--
			let arr = String(item.tuserids).split(',')
			for(let x in arr){
				if(arr[x] == this.data.user.id){
					arr.splice(x,1)
				}
	...
	async comcaiClick(e){
		...
		if(!this.comcaiChange(item)){
			item.crazilynum++
			if(item.cuserids){
				item.cuserids = item.cuserids + ',' + this.data.user.id
			}else {
				item.cuserids = this.data.user.id
			}
			api.post('discussdiscussdanciku/update', item).then((res)=>{
				utils.msg('点踩成功')
			})
			...
			item.crazilynum--
			let arr = String(item.cuserids).split(',')
			for(let x in arr){
				if(arr[x] == this.data.user.id){
					arr.splice(x,1)
				}
	...
	async onCommentTap() {
		...
		utils.jump(`../discussdiscussdanciku/add-or-update?refid=${this.data.id}`)
	...
	delClick(e){
		...
		wx.showModal({
			...
			async success(obj) {
				if(obj.confirm){
					api.post('discussdiscussdanciku/delete', JSON.stringify([id])).then((rs)=>{
						utils.msg('删除成功')
						that.data.detail.discussnum--
						api.post('discussdanciku/update', that.data.detail).then((rs2)=>{
							that.setData({
