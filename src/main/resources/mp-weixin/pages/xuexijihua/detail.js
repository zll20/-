...
Page({
	...
	async onShow() {
		...
		if(this.data.userid){
			this.setData({
				...
				isAuth: utils.isAuth('xuexijihua')
			})
		}else {
			this.setData({
				isAuth: utils.isAuthFront('xuexijihua')
			})
		}
	...
	getDetail(type=1) {
		api.get('xuexijihua/detail/' + this.data.id, {}).then((res)=>{
			let reg=new RegExp('http://localhost:8081/code/upload','g')//g代表全部
			let reg2=new RegExp('<img','g')//g代表全部

			if(type==2){
				res.data.discussnum++
				api.post('xuexijihua/update',res.data).then((rs2)=>{})
			}
			// 数据存储
			this.setData({
				detail: res.data,
				title: res.data.xingming,
			})
		})
	},
	isJSON(str) {
	...
	async onAcrossTap(e){
		...
		wx.setStorageSync('crossTable','xuexijihua');
