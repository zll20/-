...
Page({
	...
	async onLoad(options) {
		...
		if (options.id) {
			// 获取信息
			await api.get('users/info/' + options.id).then(res=>{
				ruleForm = res.data;
			})
		}
		...
		if(options.cross){
			...
			for (var o in obj){
				if(o=='username'){
					ruleForm.username = obj[o];
					ro.username = true;
					continue;
				}
				if(o=='password'){
					ruleForm.password = obj[o];
					ro.password = true;
					continue;
				}
				if(o=='role'){
	...
	async onSubmitTap(subMitType) {
		...
		//跨表计算判断
		var obj;
		if((!this.data.ruleForm.username)){
			utils.msg(`用户名不能为空`);
			return
		}
		if((!this.data.ruleForm.password)){
			utils.msg(`密码不能为空`);
			return
		}
		//更新跨表属性
		...
		if(crossrefid && crossuserid) {
			...
			await api.get('users/list',params).then(rs=>{
