...
Page({
	...
	async onShow() {
		...
		if(this.data.userid){
			this.setData({
				...
				isAuth: utils.isAuth('users')
			})
		}else {
			this.setData({
				isAuth: utils.isAuthFront('users')
			})
		}
	...
	async onAcrossTap(e){
		...
		wx.setStorageSync('crossTable','users');
