...
Page({
	...
	getDetail(type=1) {
		api.get('xuesheng/detail/' + this.data.id, {}).then((res)=>{
			let reg=new RegExp('http://localhost:8081/code/upload','g')//g代表全部
			let reg2=new RegExp('<img','g')//g代表全部

			// 轮播图片
			let swiperList = res.data.touxiang ? res.data.touxiang.split(",") : [];
			
			if(type==2){
				res.data.discussnum++
				api.post('xuesheng/update',res.data).then((rs2)=>{})
			}
			// 数据存储
			this.setData({
				detail: res.data,
	...
	async onAcrossTap(e){
		...
		wx.setStorageSync('crossTable','xuesheng');
