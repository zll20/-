...
Page({
	...
	async getList() {
		...
		if(this.data.searchForm.shifoujizhu){
			params['shifoujizhu'] = '%' + this.data.searchForm.shifoujizhu + '%'
		}
		let user = wx.getStorageSync("appUserid")?JSON.parse(wx.getStorageSync('userSession')):{}
		params.userid = wx.getStorageSync("appUserid")
		
		let url = 'dancixuexi/list'
		if(this.data.userid) {
			url = 'dancixuexi/page'
		}
	...
	onDeleteTap(e){
		...
		wx.showModal({
			...
			success: async function(res) {
				if (res.confirm) {
					await api.post('dancixuexi/delete',JSON.stringify([item.id])).then(async (rs)=>{
						utils.msg('删除成功');
						// 重置分页参数页数为1
						that.search()
					})
