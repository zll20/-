...
Page({
	...
	async onLoad(options) {
		...
		if (options.id) {
			// 获取信息
			await api.get('smsregistercode/info/' + options.id).then(res=>{
				ruleForm = res.data;
			})
		}
	...
	async onSubmitTap(subMitType) {
		...
		if((!this.data.ruleForm.role)){
			utils.msg(`角色不能为空`);
			return
		}
		if((!this.data.ruleForm.code)){
			utils.msg(`验证码不能为空`);
			return
		}
		//更新跨表属性
		...
		if(crossrefid && crossuserid) {
			...
			await api.get('smsregistercode/list',params).then(rs=>{
		...
		let oet = {}
		let url = 'smsregistercode/add'
		if(this.data.ruleForm.id){
			url = 'smsregistercode/update'
		}
