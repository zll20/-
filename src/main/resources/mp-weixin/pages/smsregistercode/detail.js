...
Page({
	...
	async onShow() {
		...
		if(this.data.userid){
			this.setData({
				...
				isAuth: utils.isAuth('smsregistercode')
			})
		}else {
			this.setData({
				isAuth: utils.isAuthFront('smsregistercode')
			})
		}
	...
	getDetail(type=1) {
		api.get('smsregistercode/detail/' + this.data.id, {}).then((res)=>{
			let reg=new RegExp('http://localhost:8081/code/upload','g')//g代表全部
			let reg2=new RegExp('<img','g')//g代表全部

			if(type==2){
				res.data.discussnum++
				api.post('smsregistercode/update',res.data).then((rs2)=>{})
			}
			// 数据存储
			this.setData({
				detail: res.data,
	...
	async onAcrossTap(e){
		...
		wx.setStorageSync('crossTable','smsregistercode');
