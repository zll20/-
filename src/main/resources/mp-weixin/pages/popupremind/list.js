...
Page({
	...
	async onShow() {
		if(wx.getStorageSync("useridTag")==1){
			this.setData({
				...
				isAuth: utils.isAuth('popupremind'),
				role: wx.getStorageSync("appRole")
			})
		}else {
			this.setData({
				isAuth: utils.isAuthFront('popupremind'),
				role: wx.getStorageSync("appRole"),
				userid: ''
			})
		}
	...
	async getList() {
		...
		let url = 'popupremind/list'
		url = 'popupremind/message/list'
	...
	onDeleteTap(e){
		...
		wx.showModal({
			...
			success: async function(res) {
				if (res.confirm) {
					await api.post('popupremind/delete',JSON.stringify([item.id])).then(async (rs)=>{
						utils.msg('删除成功');
						// 重置分页参数页数为1
						that.search()
					})
	...
	onUpdate(row){
		wx.setStorageSync("useridTag",this.data.userid);
		utils.jump(`./add-or-update?id=${row.id}`)
	},
	onDelete(row){
		var that = this;
		wx.showModal({
			...
			success: async function(res) {
				if (res.confirm) {
					await api.post('popupremind/delete',JSON.stringify(row.id)).then(async (rs)=>{
						utils.msg('删除成功');
						that.search()
					})
				}
	...
	onUpdateOrDel(e){
		...
		let that = this
		let arr = []
		if((this.data.userid && utils.isAuth('popupremind', 2, '修改')) || (!this.data.userid && utils.isAuthFront('popupremind', 2, '修改'))){
			arr.push('修改')
		}
		if((this.data.userid && utils.isAuth('popupremind', 2, '删除')) || (!this.data.userid && utils.isAuthFront('popupremind', 2, '删除'))){
			arr.push('删除')
		}
		if(!arr.length){
			return false
