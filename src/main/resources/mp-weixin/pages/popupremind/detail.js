...
Page({
	...
	getDetail(type=1) {
		api.get('popupremind/detail/' + this.data.id, {}).then((res)=>{
			let reg=new RegExp('http://localhost:8081/code/upload','g')//g代表全部
			let reg2=new RegExp('<img','g')//g代表全部
			if(res.data.content){
				res.data.content = res.data.content.replace(reg, this.data.baseurl + 'upload').replace(reg2,'<img style="width: 100%;"');
			}
			...
			if(type==2){
				res.data.discussnum++
				api.post('popupremind/update',res.data).then((rs2)=>{})
			}
			// 数据存储
			this.setData({
				detail: res.data,
	...
	async onAcrossTap(e){
		...
		wx.setStorageSync('crossTable','popupremind');
