...
Page({
	...
	async onLoad(options) {
		...
		if (options.id) {
			// 获取信息
			await api.get('config/info/' + options.id).then(res=>{
				ruleForm = res.data;
			})
		}
	...
	async onSubmitTap(subMitType) {
		...
		if(this.data.ruleForm.type&&(!validate.isIntNumer(this.data.ruleForm.type))){
			utils.msg(`类型应输入整数`);
			return
		}
		//更新跨表属性
		...
		if(crossrefid && crossuserid) {
			...
			await api.get('config/list',params).then(rs=>{
