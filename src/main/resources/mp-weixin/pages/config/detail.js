...
Page({
	...
	async onShow() {
		...
		if(this.data.userid){
			this.setData({
				...
				isAuth: utils.isAuth('config')
			})
		}else {
			this.setData({
				isAuth: utils.isAuthFront('config')
			})
		}
	...
	async onAcrossTap(e){
		...
		wx.setStorageSync('crossTable','config');
