<template>
	<el-dialog title="拍照上传" :visible.sync="visible" width="1065px">
		...
		<div slot="footer" style="display:flex">
			<el-upload :action="$base.url + 'file/upload'" :on-success="uploadSuccess" :show-file-list="false"
...
<script>
	export default {
		...
		methods: {
			...
			sendFrame() {
				// 只有当状态为 detecting 且 Socket 连接正常时才发送
				var canvas = document.getElementById('canvasCamera')
				// 压缩：将画面缩小到 320x240 发送，大幅降低延迟
				// this.ctx.drawImage(localVideo, 0, 0, 320, 240);
			...
			getCompetence() {
				...
				this.$nextTick(() => {
					...
					let baseUrl = this.baseUrl.replace(/^http/, 'ws')
					// 注意：匹配后端的路由
					const wsUrl = `${baseUrl}yonghu/faceLogin/live`;
