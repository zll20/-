		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 30px 0","fontSize":"inherit","borderRadius":"8px","background":"non"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"0px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('config/' + configStatus,'新增')&&configStatus==1" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#FA6B04","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('config/' + configStatus,'删除')&&configStatus==1" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"20px ","borderRadius":"8px","background":"#fff","borderWidth":"0"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('config/' + configStatus,'查看')"
					:data="dataList"
					...
					<el-table-column :resizable='true' :sortable='true' prop="name" label="名称">
						<template slot-scope="scope">
							{{ changeBgName(scope.row.name) }}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="value" width="200" label="图片">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if="isAuth('config/' + configStatus,'查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if="isAuth('config/' + configStatus,'修改')" type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('config/' + configStatus,'删除')&&configStatus==1" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
	...
	export default {
		data() {
			return {
				...
				configStatus: this.$route.params.type,
				bgNameList: this.$getBgName.bgNameList,
				previewImg: '',
		...
		watch: {
			'$route' (to, from) { //监听路由是否变化
				if(this.$route.params.type){//判断状态是否有值
				//调数据
		...
		methods: {
			changeBgName(e) {
				...
				for(let x in this.bgNameList) {
					if(this.bgNameList[x].value==e) {
						return this.bgNameList[x].name
					}
				}
			},
			queryChange(arr){
			...
			addOrUpdateHandler(id,type) {
				...
				this.$nextTick(() => {
					this.$refs.addOrUpdate.init(id,type ,this.dataList.length);
				});
			},
			// 查看评论
			...
			async deleteHandler(id ) {
				if((!id&&(this.dataList.length - this.dataListSelections.length)<3)||(id&&this.dataList.length<=3)) {
					this.$message.error('轮播图不能小于三个')
					return false
				}
