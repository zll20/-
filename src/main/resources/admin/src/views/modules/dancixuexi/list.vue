...
<template>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 30px 0","fontSize":"inherit","borderRadius":"8px","background":"non"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 10px 0px","boxShadow":"none","borderColor":"#00000050","borderRadius":"0px","flexWrap":"wrap","borderWidth":"0","display":"flex","width":"100%","fontSize":"inherit","position":"relative","borderStyle":"solid"}' >
					<div  :style='{"margin":"0 1% 20px 0","fontSize":"inherit","display":"flex"}' class="select" label="是否记住" prop="shifoujizhu">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#333333","display":"inline-block","lineHeight":"41px","fontSize":"inherit","fontWeight":"500","height":"41px"}' class="item-label">是否记住</label>
						<el-select clearable v-model="searchForm.shifoujizhu" placeholder="请选择是否记住" >
							<el-option v-for="(item,index) in shifoujizhuOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"0px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('dancixuexi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#FA6B04","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('dancixuexi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 0px","fontSize":"16px","color":"#FA6B04","height":"auto"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('dancixuexi','学习分析')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#FA6B04","height":"auto"}'></span>
						学习分析
					</el-button>
					<el-button class="btn18" v-if="isAuth('dancixuexi','学习时长')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"16px","color":"#FA6B04","height":"auto"}'></span>
						学习时长
					</el-button>
				</el-row>
          ...
          <el-table-column :resizable='true' :sortable='true'
                           ...
                           label="单词量">
            <template slot-scope="scope">
              {{scope.row.danciliang}}
            </template>
          </el-table-column>
          <el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('dancixuexi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('dancixuexi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('dancixuexi','删除')" type="primary" @click="deleteHandler(scope.row.id)">
    ...
    <el-dialog
        ...
        width="800">
      <div id="shifoujizhuChart1" style="width:100%;height:600px;"></div>
    ...
    <el-dialog
        ...
        width="800">
      <div id="xuexishizhangChart2" style="width:100%;height:600px;"></div>
      <span slot="footer" class="dialog-footer">
    ...
    return {
      ...
      showFlag: true,
		...
		methods: {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var shifoujizhuChart1 = echarts.init(document.getElementById("shifoujizhuChart1"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							shifoujizhuChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shifoujizhuChart1.resize();
							};
						}
					})
				})
			...
			chartDialog2() {
				this.$nextTick(()=>{
					var xuexishizhangChart2 = echarts.init(document.getElementById("xuexishizhangChart2"),'macarons');
					let params = {}
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							xuexishizhangChart2.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								xuexishizhangChart2.resize();
							};
						}
					})
				})
			...
			getDataList() {
				...
				if(this.searchForm.shifoujizhu!='' && this.searchForm.shifoujizhu!=undefined){
