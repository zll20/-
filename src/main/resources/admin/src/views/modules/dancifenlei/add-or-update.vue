...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="单词分类" prop="dancifenlei" >
					<el-input v-model="ruleForm.dancifenlei" placeholder="单词分类" clearable  :readonly="ro.dancifenlei"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="单词分类" prop="dancifenlei" >
					<el-input v-model="ruleForm.dancifenlei" placeholder="单词分类" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `dancifenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.dancifenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
