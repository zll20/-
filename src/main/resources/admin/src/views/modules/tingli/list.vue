		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 30px 0","fontSize":"inherit","borderRadius":"8px","background":"non"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"width":"calc(100% + 8px)","padding":"0px","margin":"0px 0 0px -4px","flexWrap":"wrap","justifyContent":"0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('tingli','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"margin":"0 0px","fontSize":"16px","color":"#FA6B04","height":"auto"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('tingli','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"20px ","borderRadius":"8px","background":"#fff","borderWidth":"0"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="shiyinshipin" width="200" label="听力音频">
						<template slot-scope="scope">
							<div v-if="scope.row.shiyinshipin">
								<audio v-if="scope.row.shiyinshipin.substring(0,4)=='http'" :key="'list-http-audio-'+scope.row.id" :src="scope.row.shiyinshipin" controls style="width: 150px;"></audio>
								<audio v-else :key="'list-local-audio-'+scope.row.id" :src="$base.url+scope.row.shiyinshipin" controls style="width: 150px;"></audio>
							</div>
							<div v-else>无音频</div>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('tingli','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"#333","display":"none","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('tingli','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('tingli','删除')" type="primary" @click="deleteHandler(scope.row.id)">
