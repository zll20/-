	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="学习任务" prop="xuexirenwu" >
					<el-input v-model="ruleForm.xuexirenwu" placeholder="学习任务" clearable  :readonly="ro.xuexirenwu"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="学习任务" prop="xuexirenwu" >
					<el-input v-model="ruleForm.xuexirenwu" placeholder="学习任务" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="复习计划" prop="fuxijihua" >
					<el-input v-model="ruleForm.fuxijihua" placeholder="复习计划" clearable  :readonly="ro.fuxijihua"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="复习计划" prop="fuxijihua" >
					<el-input v-model="ruleForm.fuxijihua" placeholder="复习计划" readonly></el-input>
				</el-form-item>
			</template>
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					this.ruleForm.xuexishizhang = Number('2'); 				}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xuexijihua/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xuexijihuaCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
